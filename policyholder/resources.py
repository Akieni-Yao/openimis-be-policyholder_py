...
class InsureeResource(resources.ModelResource):
    ...
    chf_id = fields.Field(
        attribute='chf_id',
        column_name='Tempoprary CAMU Number',
    )
    camu_number = fields.Field(
        attribute='camu_number',
        column_name='CAMU Number',
    )
    ...
    # )
    # Civility = fields.Field(
    #     attribute='Civility',
    #     column_name='Civilité',
    # )
    # phone = fields.Field(
    #     attribute='phone',
    #     column_name='Téléphone',
    # )
    ...
    # family = fields.Field(
    #     attribute='family',
    #     column_name='ID Famille',
    #     widget=ForeignkeyRequiredWidget(Family, field='id'),
    #     readonly=True,
    # )
    # email = fields.Field(
    #     attribute='email',
    ...
    def __init__(self, user, policy_holder, queryset=None, ):
        """
    ...
    def get_queryset(self):
        ...
        insuree_ids = PolicyHolderInsuree.objects.filter(policy_holder__code=self._policy_holder, policy_holder__date_valid_to__isnull=True, 
                                                        policy_holder__is_deleted=False, date_valid_to__isnull=True, 
                                                        is_deleted=False).values_list('insuree_id', flat=True).distinct()
        return queryset \
            .filter(validity_to__isnull=True, id__in=insuree_ids) \
    ...
    def get_place_of_birth(self, instance):
        json_data = instance.json_ext
        return json_data['BirthPlace'] if json_data['BirthPlace'] else ""
