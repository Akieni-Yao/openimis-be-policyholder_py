...
headers = {"Content-Type": "application/json", "Tmr-Api-Key": "test"}
...
def erp_create_update_policyholder(ph_id, cpb_id, user):
    ...
    if phcp and phcp.policy_holder.bank_account:
        ...
        if account_no:
            ...
            bank_details = Banks.objects.filter(
                code=bank_code, is_deleted=False
            ).first()
    ...
    try:
        ...
        PolicyHolder.objects.filter(id=ph_id).update(
            erp_partner_id=response_json.get("id"),
            erp_partner_access_id=response_json.get("partner_access_id"),
        )
...
def erp_delete_policyholder(ph_id, cpb_id, user):
    ...
    logger.debug(
        f" ======    erp_delete_policyholder : response.text : {response.text}    ======="
    )
    ...
    if response.status_code not in [200, 201]:
        ...
        try:
            ...
            logger.error(f"Failed to delete ERP API Failed log: {e}")
    # try:
    #     response_json = response.json()
    #     logger.debug(
    #         f" ======    erp_create_update_policyholder : response.json : {response_json}    ======="
...
def erp_create_update_fosa(policyholder_code, account_payable_id, user):
    ...
    payment = Payment.objects.filter(
        contract__policy_holder__code=policyholder_code
    ).first()

    health_facility = HealthFacility.objects.filter(
        legacy_id__isnull=True,
        validity_to__isnull=True,
        json_ext__camuCode=policyholder_code,
    ).first()
    bank_accounts = None
    if phcp and phcp.policy_holder.bank_account:
        bank_account = phcp.policy_holder.bank_account.get("bankAccount", {})
...
def erp_delete_fosa(policyholder_code, account_payable_id, user):
    ...
    logger.debug(f" ======    erp_delete_fosa : url : {url}    =======")
    logger.debug(
        f" ======    erp_delete_fosa : policyholder_data : {policyholder_data}    ======="
    )
    ...
    logger.debug(
        f" ======    erp_delete_fosa : response.text : {response.text}    ======="
    )
    ...
    try:
        response_json = response.json()
        logger.debug(
            f" ======    erp_delete_fosa : response.json : {response_json}    ======="
        )
    except json.JSONDecodeError:
        logger.error("Failed to decode JSON response")

    logger.debug(" ======    erp_delete_fosa - end    =======")
    return True
