...
urlpatterns = [
    ...
    path(
        "verify-email/<uidb64>/<token>/<e_timestamp>/",
        views.verify_email,
        name="verify_email",
    ),
    ...
    path(
        "custom-policyholder-policies-expire", views.custom_policyholder_policies_expire
    ),
    path(
        "tipl/get-declaration-details/<policy_holder_code>",
        views.get_declaration_details,
    ),
    path("tipl/paid-contract-payment", views.paid_contract_payment),
    path(
        "create-existing-policyholder-in-erp/",
        erp_intigration.create_existing_policyholder_in_erp,
    ),
